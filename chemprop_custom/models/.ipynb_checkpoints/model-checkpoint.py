...
rom chemprop_custom.data import BatchMolGraph, TrainingBatch
from chemprop_custom.nn import Aggregation, ChempropMetric, MessagePassing, Predictor
from chemprop_custom.nn.transforms import ScaleTransform
from chemprop_custom.schedulers import build_NoamLike_LRSched
...
class CustomLoss(nn.Module):
    ...
    # Calculate cosine similarity and handle zero division error
    if magnitude_vec1 == 0 or magnitude_vec2 == 0:
        return 0.0
    else:
        return dot_product / (magnitude_vec1 * magnitude_vec2)

    def cosine_similarities_between_vectors(vector1, vector2):
    ...
    def forward(self, output, rationales, plausible):
        ...
        return torch.mean(torch.sum(cosine_similarity(output, rationales)*, dim=1))
...
class EncoderTrainer(pl.LightningModule):
    ...
    def fingerprint(
        ...
        return self.mpnn.fingerprint(bmg, V_d, X_d)

    def forward(
        self, bmg: BatchMolGraph, V_d: Tensor | None = None, X_d: Tensor | None = None
    ) -> Tensor:
        return self.fingerprint(bmg, V_d, X_d)

    def training_step(self, batch: TrainingBatch, batch_idx):
        bmg, V_d, X_d, target_smiles, booleans = batch

        Z = self(bmg, V_d, X_d)
        target_encoding = self(target_smiles, V_d, X_d)
        loss = self.criterion(Z, target_encoding)
    ...
    def validation_step(self, batch: TrainingBatch, batch_idx: int = 0):
        ...
        self.log("val_loss", loss, batch_size=len(batch[0]), prog_bar=True)
    ...
    def test_step(self, batch: TrainingBatch, batch_idx: int = 0):
        ...
        self.log("test_loss", loss, batch_size=len(batch[0]))

    def configure_optimizers(self):
        # Only optimize the encoder parts
        encoder_params = list(self.mpnn.message_passing.parameters()) + \
                         list(self.mpnn.agg.parameters()) + \
                         list(self.mpnn.bn.parameters())
        
        opt = optim.Adam(encoder_params, self.init_lr)
...
# After training, you can use the MPNN end-to-end as before
def use_mpnn_end_to_end(mpnn: MPNN, input_data):
    return mpnn(input_data)
