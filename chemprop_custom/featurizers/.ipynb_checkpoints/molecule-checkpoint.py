...
from chemprop_custom.featurizers.base import VectorFeaturizer
from chemprop_custom.utils import ClassRegistry
...
class RDKit2DFeaturizer(VectorFeaturizer[Mol]):
    ...
    #def __call__(self, mol: Chem.Mol) -> np.ndarray:
    def __call__(self, mol: Chem.Mol, V_f: np.ndarray | None = None, E_f: np.ndarray | None = None) -> np.ndarray:
