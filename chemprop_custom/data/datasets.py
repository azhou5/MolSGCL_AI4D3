...
from chemprop_custom.data.datapoints import MoleculeDatapoint, ReactionDatapoint
from chemprop_custom.data.molgraph import MolGraph
from chemprop_custom.featurizers.base import Featurizer
from chemprop_custom.featurizers.molgraph import CGRFeaturizer, SimpleMoleculeMolGraphFeaturizer
from chemprop_custom.featurizers.molgraph.cache import MolGraphCache, MolGraphCacheOnTheFly
from chemprop_custom.types import Rxn
...
class Datum(NamedTuple):
    ...
    rationale_mg: MolGraph | None
    neg_rationale_mg: MolGraph | None
    plausible: bool | None
    neg_plausible: bool | None
    secondary_target: np.ndarray | None
...
class _MolGraphDatasetMixin:
    ...
    def rationales(self) -> list[np.ndarray]:
        """the (scaled) atom descriptors of the dataset"""
        return self.__rationales
    def plausibility(self) -> list[np.ndarray]:
        """the (scaled) atom descriptors of the dataset"""
        return self.__plausibility

    @property 
    def secondary_target(self) -> np.ndarray:
        """the (scaled) atom descriptors of the dataset"""
        return self.__secondary_target
    @secondary_target.setter
    def secondary_target(self, secondary_target: ArrayLike):
        self._validate_attribute(secondary_target, "secondary_targets")
        self.__secondary_target = np.array(secondary_target, float)
    @Y.setter
...
class MoleculeDataset(_MolGraphDatasetMixin, MolGraphDataset):
    ...
    def __getitem__(self, idx: int) -> Datum:
        ...
        return Datum(
            ...
            lt_mask=d.lt_mask,
            gt_mask=d.gt_mask,
            rationale_mg=d.rationale_mg,
            neg_rationale_mg=d.neg_rationale_mg,
            plausible=d.plausible,
            neg_plausible=d.neg_plausible,
