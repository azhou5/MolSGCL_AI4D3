...
class _DatapointMixin:
    ...
    rationale_mol: Chem.Mol | None = None
    neg_rationale_mol: Chem.Mol | None = None
    plausible: bool = False
    neg_plausible: bool = False
    mg: MolGraph | None = None
    rationale_mg: MolGraph | None = None
    neg_rationale_mg: MolGraph | None = None
    secondary_target: np.ndarray | None = None
...
class _ReactionDatapointMixin:
    ...
    def from_smi(
        ...
        **kwargs,
        ...
        # Support both string and tuple inputs without requiring Python 3.10 pattern matching
        if isinstance(rxn_or_smis, str):
            rct_smi, agt_smi, pdt_smi = rxn_or_smis.split(">")
            rct_smi = f"{rct_smi}.{agt_smi}" if agt_smi else rct_smi
            name = rxn_or_smis
        elif isinstance(rxn_or_smis, tuple):
