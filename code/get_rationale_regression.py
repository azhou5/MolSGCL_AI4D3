...
try:
    # switch to forkserver (or 'fork') to avoid per-spawn tempdirs
    multiprocessing.set_start_method('forkserver')
except RuntimeError:
    # already set in another part of your code
    pass
...
max_atoms = 11 # maximum number of atoms allowed in an extracted rationale
min_atoms = 5 # minimum number of atoms in an extracted rationale
prop_delta = -10  # set low so that all rationales are collected. 
...
def process_single_smiles(smiles, scoring_func_base, rollout_val, max_atoms_val, prop_delta_val, min_atoms_val, c_puct_val, num_rationales, property_name, model_path_for_worker=None, model=None):
    ...
    if torch.cuda.is_available():
        # Decide if workers should use GPU or CPU. If multiple workers use GPU,
        # ensure your GPU has enough memory. Often, it's better for workers
        # to use CPU unless the model inference is very heavy and GPU memory allows.
        # For simplicity here, let's assume workers use CPU if CUDA is available
        # but you might adjust this based on your hardware.
        # For simplicity here, let's assume workers use CPU if CUDA is available
        # but you might adjust this based on your hardware.
        worker_map_location = {'cuda:0': 'cpu'}      # If workers should use CPU
    ...
    # Find rationales if score is above threshold (for regression, we check all molecules above prop_delta)
...
def run_MCTS(smiles_list, output_dir='rationale_finding', num_cores=None, property_name="Y", model_path=None, model=None, disable_multiprocessing=False, prop_delta_threshold=0.5):
    """
    ...
    prop_delta_threshold : float
        Minimum property value threshold for considering rationale extraction (for regression)
    """
    # Set the global prop_delta to the user-specified threshold
    global prop_delta
    prop_delta = prop_delta_threshold
    ...
    if len(smiles_list) <= 5 or disable_multiprocessing:
        print(f"Using single processing for {len(smiles_list)} molecules with prop_delta threshold: {prop_delta}")
        results = []
    ...
    print(f"Using {num_cores} CPU cores for parallel processing of {len(smiles_list)} molecules with prop_delta threshold: {prop_delta}")
    ...
    # Use default context (should be forkserver if set above)
...
def main():
    parser = argparse.ArgumentParser(description='Run MCTS rationale extraction on SMILES strings from a CSV file for regression tasks.')
    ...
    parser.add_argument('--prop_delta', type=float, default=0.5, help='Minimum property value threshold for rationale extraction (default: 0.5)')
    ...
    print(f"Processing {len(smiles_list)} molecules with property threshold: {args.prop_delta}")
    ...
    results = run_MCTS(
        ...
        prop_delta_threshold=args.prop_delta
    )
    ...
    molecules_with_rationales = results[~results['rationale_0'].isna()]
    print(f"\nSummary:")
    print(f"Total molecules processed: {len(results)}")
    print(f"Molecules with rationales found: {len(molecules_with_rationales)}")
    print(f"Percentage with rationales: {len(molecules_with_rationales)/len(results)*100:.1f}%")
    if len(molecules_with_rationales) > 0:
        print(f"Average property value for molecules with rationales: {molecules_with_rationales[args.property_name].mean():.3f}")
